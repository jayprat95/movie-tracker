...
public class TextFileParser {
	...
	/**
	 * This method puts the list into a long string with a comma delimiter in
	 * order to store the string in memory easier.
	 * 
	 * @param list
	 *            the list to put into a string
	 * @return the single string
	 */
	public String getStringFromListHelper(ArrayList<String> list) {
	 ...
	 */
	public ArrayList<String> alphabetize(ArrayList<String> theList) {
	...
	// ----------------------------------------------------------
	/**
	 * This method stores a list of titles in internal memory. i.e.
	...
	public void setMovieList(ArrayList<String> moviesToSet, String listname) {
		...
		ArrayList<String> moviesToSetAlphabetized = this
				.alphabetize(moviesToSet);

		// Create output stream with spot in internal memory or overwrite if
		// file already exists.
		FileOutputStream fos = null;
		try {
			fos = context.openFileOutput("STORED_" + listname + ".txt",
		...
		try {
			fos.write(getStringFromListHelper(moviesToSetAlphabetized)
					.getBytes());
			fos.flush();
			fos.close();
		} catch (IOException e) {
	...
	public void parseStringsToStringList(String filename) {
		...
		if ((filename.equals("watched")) && (watchedString.length() > 0)) {
			List<String> watchedTemp = Arrays.asList(watchedString
					.split(delimiter));
		...
		if ((filename.equals("toWatch")) && (toWatchString.length() > 0)) {
			List<String> toWatchTemp = Arrays.asList(toWatchString
					.split(delimiter));
		...
		if ((filename.equals("favorite")) && (favoriteString.length() > 0)) {
			List<String> favoriteTemp = Arrays.asList(favoriteString
					.split(delimiter));
